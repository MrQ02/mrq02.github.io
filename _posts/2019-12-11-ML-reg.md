...
- [Ridge Regression](#ridge)
    - [Problem Setting](#ridgeprob)
    - [Model](#ridgemodel)
    - [Solution](#ridgesol)
    - [Preprocessing](#ridgeprep)
...
- <a name="ridgemodel"></a>**Model**

    $$\begin{equation}
    w_{RR}=\mathop{\arg\min}_ {w}\|{y-Xw}\|^2+\lambda\|w\|^2
    \end{equation}$$
    ...
    - $g(w)=\\|w\\|^2$: penalty function
    
- <a name="ridgesol"></a>**Solution**

    $$\begin{align}
    \mathcal{L}&=(y-Xw)^T(y-Xw)+\lambda w^Tw \\
    \nabla_w\mathcal{L}&=-2X^Ty+2X^TXw+2\lambda w=0 \\
    \Rightarrow w_{RR}&=(X^TX+\lambda I)^{-1}X^yT
    \end{align}$$
    
- <a name="ridgeprep"></a>**Data Preprocessing: Standardization**
    ...
    - $x$:
        ...
        $$\begin{equation}
        x_{ij}\leftarrow \frac{x_{ij}-\bar{x}_ j}{\sqrt{\frac{1}{n}\sum_{i=1}^n{(x_{ij}-\bar{x}_ j)^2}}}
        \end{equation}$$
